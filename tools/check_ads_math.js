...
// --- クリエイティブ別ランキングの約束ごと ---------------------------
{
  ...
  // 広告単位の分母（プロフアクセス数・リンク遷移数）が取り込まれたので、
  // クリエイティブ別にもCPCを出せる。クリック単価とは別の指標なので列を分ける
  has(/\{k:"cpcv",name:"CPC",\s*val:e=>\(e\.dnn&&\(e\.pv\+e\.lp\)>0\)\?e\.sp\/\(e\.pv\+e\.lp\):null/,
    "クリエイティブ別にCPCの列がある（分母は プロフアクセス数＋リンク遷移数）");
  has(/if\(r\.pv!=null\|\|r\.lp!=null\)\{ e\.pv\+=r\.pv\|\|0; e\.lp\+=r\.lp\|\|0; e\.dnn\+\+; \}\n\s*e\.days\.add/,
    "CPCの分母は取り込めた日だけ数える（0で薄めない）");
  // 名前が同じだと取り違える。CPCとクリック単価は別の列として並べる
  has(/\{k:"cpc",name:"クリック単価"/, "クリック単価も残す（別の指標）");
  // need の形が増えた（{any:[...]}）。列の出し分けが対応していないと常に「出せる」になる
  has(/const adsHas=need=>\{/, "広告単位のデータ有無を見る関数がある");
  has(/if\(need\.any\)return need\.any\.some\(some\);/, "{any:[...]} はどちらか一方でも来ていれば出す");
  has(/const ready=c=>adsHas\(c\.need\);/, "列の出し分けはその関数を通す");
}
...
//   割れないものは全社のページにしか置かない、という切り分けを見張る。
{
  ...
  // 広告は売上アプリのタブではなくなった（独立したページ）。
  // 売上のタブバーに広告が並んでいたら、また中に取り込まれている
  if (/id="tab-ads"/.test(src)) {
    console.log("  NG  売上のタブバーに広告が戻っています"); ng++;
  } else console.log("  OK  売上のタブバーに広告は無い（独立したページ）");
  // 広告ページを開いているあいだは、売上のタブバー自体を出さない
  has(/広告は売上アプリの中のタブではなく、独立したページとして扱う/,
      "広告ページでは売上のタブバーを隠している");
  // 広告は店舗のキャンペーンの中で走るので、1本の広告は必ず1店に属する。
  // だからクリエイティブも区分で割れる＝全社だけのページは要らない
  // 見出しに属性（英字の小見出し用の data-en）が付いても通るようにしてある。
  // 見たいのは「区分ごとに出しているか」であって、タグの書き方ではない
...
// --- どこからも呼ばれていない関数が残っていないか ----------------------
//   パネルを消したときに、それを描く関数だけが残ることがある。無害なうちはいいが、
//   「消したはずの画面のコード」を読んで混乱する／あとで誰かが呼んで落ちる。
{
  ...
  if (dead.length) {
    console.log(`  NG  どこからも呼ばれていない関数が ${dead.length}個 残っています`);
