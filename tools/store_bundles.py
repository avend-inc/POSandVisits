...
def _sum_rows(rows, kv_of, visit_stores):
    ...
    p = {"in": 0.0, "ex": 0.0, "tx": 0, "txv": 0, "it": 0.0, "txIt": 0,
         "v": 0, "bagEx": 0.0, "komEx": 0.0, "hasV": False, "hasEx": False, "hasIt": False}
    for r in rows:
        p["in"] += r.get("in") or 0
        p["tx"] += r.get("tx") or 0
        ...
        # レジ袋・小物の税抜（商品単価の分子から差し引く用。dashboard.html と同じ定義）
...
def _calc(k, p):
    ...
    if k == "aup":
        # 商品単価＝(税抜売上−レジ袋−小物)÷点数。分母(点数)がレジ袋・小物を除くので分子も揃える。
        return ((p["ex"] - p["bagEx"] - p["komEx"]) / p["it"]) if (p["hasEx"] and p["it"] > 0) else None
    return None
