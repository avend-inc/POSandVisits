...
- デジテール: 山形/いわき/福井 各 2026-03-13〜07-26（136日）。
- いわきは cashier 上2店舗登録だが adapters が「いわき」に正規化し自動合算。
- 二重登録は sales/visits の一意キーで無視（何度流しても安全）。ingest_log は汚さない。
- **cashierの期間欄は name="since"(開始)/"until"(終了)**。ここを名前で指定＋JSセット
  しないと「今日1日」になる落とし穴があった（対応済み）。
